...
//新窗口打开链接
function sz_open_window(url, name) {
	name || '';
...
$(function() {
	...
	$("div.tab_content").children("div").addClass("sz_tab_content_div");
	//点击切换
...
$.extend({
	...
	"dialog" : function(options) {
		...
		if ($("div.dialog").length == 0) {
			//移除dialog_message对话框
			$('#sz_dialog_message').remove();
			$('#sz_dialog_message_bg').remove();
			var div = '';
...
function dialog_message(message, timeOut) {
	...
	if (message === false) {
		...
		//创建背景色
		var html = '<div id="sz_dialog_message_bg"></div>';
		html += "<div id='sz_dialog_message'>" + message + "</div>";
		$("body").append(html);
		//改变背景色
		$("div#sz_dialog_message_bg").css({
			opacity : 0.8
		});
		setTimeout(function() {
			$("div#sz_dialog_message").fadeOut("fast");
			$("div#sz_dialog_message_bg").remove();
		}, timeOut);
	}
}
...
$.extend({
	"modal" : function(options) {
		...
		div += '<div class="sz-modal" style="position:fixed;left:50%;top:50px;margin-left:-' + (opt['width'] / 2) + 'px;width:' + opt['width'] + 'px;' + show + 'height:' + opt['height'] + 'px;z-index:1000">';
		//---------------标题设置
		if (opt['title']) {
			div += '<div class="sz-modal-title">' + opt['title'];
			//---------x关闭按钮
		...
		if (opt.message) {
			div += '<div class="sz-modal-message"><strong class="' + opt.type + '"></strong><span>' + opt.message + '</span></div>';
		} else {
			div += opt.content;
		}
		div += '</div>';
		//------------按钮处理
		if (opt.button_success || opt.button_cancel) {
			div += '<div class="sz-modal-footer" ' + (opt.message ? 'style="text-align:center"' : "") + '>';
			//确定按钮
			if (opt.button_success) {
				div += '<a href="javascript:;" class="btn btn-primary sz-success">' + opt.button_success + '</a>';
			}
			//放弃按钮
			if (opt.button_cancel) {
				div += '<a href="javascript:;" class="btn sz-cancel">' + opt.button_cancel + '</a>';
			}
			div += '</div>';
		}
		div += '</div>';
		div += '<div class="sz-modal-bg" style="' + show + '"></div>';
		$(div).appendTo("body");
		var pos = center_pos($(".modal"));
		//点击确定
		$("div.sz-modal-footer a.sz-success").click(function() {
			if (opt.success) {
				opt.success();
			} else {
				$("div.sz-modal-footer a.sz-cancel").trigger("click");
			}
			return true;
		})
		var _w = $(document).width();
		...
		if (opt.show) {
			$("div.sz-modal-bg").show();
		}
		//点击关闭modal
		if (opt.cancel) {
			$("div.sz-modal-footer a.sz-cancel").live("click", opt.cancel);
		} else {
			$("div.sz-modal-footer a.sz-cancel").bind("click", function() {
				$.removeModal();
				return false;
			})
		}
	...
	"removeModal" : function() {
		$("div.sz-modal").fadeOut().remove();
		$("div.sz-modal-bg").remove();
	},
...
function sz_confirm(message, success, error) {
	return $.modal({
		width : 280,
		height : 160,
		title : "温馨提示",
 ...
 * @returns {boolean}
 */
function sz_submit(obj, url, func,timeout) {
	if ($(obj).is_validate()) {
		//阻止多次表单提交，提交结束后在sz_submit方法中解锁
 ...
 * @returns {boolean}
 */
function sz_ajax(requestUrl, postData, url) {
	$.ajax({
		type : "POST",
		url : requestUrl,
		cache : false,
 ...
 * @returns {boolean}
 */
function sz_submit_confirm(obj, url) {
	if ($(obj).is_validate()) {
...
$.fn.extend({
	validate : function(options) {
		...
		$(this).submit(function(event, action) {
			//如果是通过sz_submit提交时，此方法失效
			if ($(this).attr('sz_submit'))
				return false;
