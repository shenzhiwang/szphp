...
class ViewCompile
{
    ...
    public function run()
    {
        ...
        $this->content = '<?php if(!defined("SZPHP_PATH"))exit;C("SHOW_NOTICE",FALSE);?>' . $this->content;
        if (!is_dir(COMPILE_PATH)) {
            Dir::create(COMPILE_PATH);
            copy(SZPHP_TPL_PATH . 'index.html', COMPILE_PATH . 'index.html');
        }
    ...
    private function loadParseTags()
    {
        ...
        //加载框架核心标签库
        if (import('SZPHP.Lib.Driver.View.ViewTag')) {
            $tagClass[] = 'ViewTag';
