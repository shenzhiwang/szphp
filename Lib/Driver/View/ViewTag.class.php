...
class ViewTag
{
    ...
    public function _zoom($attr, $content)
    {
        ...
        $swfupload_path = __SZPHP_EXTEND__ . '/Org/Jqzoom'; //插件目录
        $str = '';
    ...
    {
        $uploadify_url = __SZPHP_EXTEND__ . '/Org/Uploadify/'; //uploadify目录
        static $_sz_uploadify_js = false; //深知网js文件只加载一次
...
<input type=\'text\'  value=\'".$f["alt"]."\' name=\'' . $name . '[".$upFileId."][alt]\'>
        ...
        if (!$_sz_uploadify_js) {
            ...
            <script type="text/javascript">
            var SZPHP_CONTROL         = "' . $phpScript . '&g=' . GROUP_NAME . '";
            var UPLOADIFY_URL    = "' . $uploadify_url . '";
            var SZPHP_UPLOAD_THUMB    ="' . $thumb . "\";\n";
            //已经成功上传的文件
            $uploadTotal = 0;
            if ($data && isset($view->vars[$data])) {
            ...
            $str .= 'SZPHP_UPLOAD_TOTAL = ' . $uploadTotal;
            $str .= '</script>
            <script type="text/javascript" src="' . $uploadify_url . 'sz_uploadify.js"></script>';
        }

        $str .= '
<script type="text/javascript">
    $(function() {
        sz_uploadify_options.removeTimeout  =' . $removeTimeout . ';
        sz_uploadify_options.fileSizeLimit  ="' . $size . '";
        sz_uploadify_options.fileTypeExts   ="' . $type . '";
        sz_uploadify_options.queueID        ="' . $queueclose . '";
        sz_uploadify_options.showalt        =' . $alt . ';
        sz_uploadify_options.uploadLimit    =' . $limit . ';
        sz_uploadify_options.input_type    ="' . $_input_type . '";
        sz_uploadify_options.elem_id    ="' . $_elem_id . '";
        sz_uploadify_options.upload_img_max_width    ="' . $upload_img_max_width . '";
        sz_uploadify_options.upload_img_max_    ="' . $upload_img_max_height . '";
        sz_uploadify_options.success_msg    ="正在上传...";//上传成功提示文字
        sz_uploadify_options.formData       ={' . $_post . 'water : "' . $water . '",upload_img_max_width:"' . $upload_img_max_width . '",upload_img_max_height:"' . $upload_img_max_height . '", someOtherKey:1,' . C("SESSION_NAME") . ':"' . session_id() . '",upload_dir:"' . $upload_dir . '",SZPHP_upload_thumb:"' . $thumb . '"};
        sz_uploadify_options.thumb_width          =' . $width . ';
        sz_uploadify_options.thumb_height          =' . $height . ';
        sz_uploadify_options.uploadsSuccessNums = ' . $uploadsSuccessful . ';
        $("#' . $id . '").uploadify(sz_uploadify_options);
        });
</script>
...
';
        if ($_waterbtn) {
            $str .= '<input type="checkbox" id="add_upload_water" uploadify_id="sz_uploadify_' . $_name . '" ' . ($water ? "checked='checked'" : "") . '/><strong style="color:#03565E">是否添加水印</strong>';
        }
...
<div class="' . $id . '_files uploadify_upload_files" input_file_id ="' . $id . '">
    ...
    {
//        $ueditor_path = __SZPHP_EXTEND__ . '/Org/Editor/Ueditor/'; //url路径
        $attr = array_change_key_case_d($attr, 0);
        ...
        if (!defined("HD_UEDITOR")) {
            $str .= '<script type="text/javascript" charset="utf-8" src="' . __SZPHP_EXTEND__ . '/Org/Ueditor/ueditor.config.js"></script>';
            $str .= '<script type="text/javascript" charset="utf-8" src="' . __SZPHP_EXTEND__ . '/Org/Ueditor/ueditor.all.min.js"></script>';
            $str .= '<script type="text/javascript">UEDITOR_HOME_URL="' . __SZPHP_EXTEND__ . '/Org/Ueditor/"</script>';
            define("HD_UEDITOR", true);
        }
        $str .= '<script id="sz_' . $name . '" name="' . $name . '" type="text/plain">' . $initContent . '</script>';
        $app_group = GROUP_NAME;
        $str .= "
        <script type='text/javascript'>
        $(function(){
                var ue = UE.getEditor('sz_{$name}',{
    ...
    public function _keditor($attr, $content)
    {
        ...
        if (!defined("keditor_hd")) {
            $str .= '<script charset="utf-8" src="' . __SZPHP_EXTEND__ . '/Org/Keditor/kindeditor-all-min.js"></script>
            <script charset="utf-8" src="' . __SZPHP_EXTEND__ . '/Org/Keditor/lang/zh_CN.js"></script>';
            define("keditor_hd", 1);
        }
        $session = session_name() . '=' . session_id();
        $option_var = str_replace(array('[', ']'), '', $name);
        $str .= '
        <textarea id="sz_' . $option_var . '" name="' . $name . '">' . $content . '</textarea>
        <script>
        ...
        $str .= 'var sz_' . $option_var . ';
        KindEditor.ready(function(K) {
                    sz_' . $option_var . ' = KindEditor.create("#sz_' . $option_var . '",options_' . $option_var . ');
        });
        </script>
        ';
        return $str;
    ...
    {
        return '<link type="text/css" rel="stylesheet" href="__SZPHP_EXTEND__/Org/Editor/Keditor/plugins/code/prettify.css"/>
        <script type="text/javascript" charset="utf-8" src="__SZPHP_EXTEND__/Org/Ueditor/third-party/codemirror/codemirror.js"></script>
    <link rel="stylesheet" type="text/css" href="__SZPHP_EXTEND__/Org/Ueditor/third-party/codemirror/codemirror.css"/>
    <script>
  var editor = CodeMirror.fromTextArea(myTextarea, {
    ...
    {
        $str = '';
        $str .= "<link href='__SZPHP_EXTEND__/Org/bootstrap/css/bootstrap.min.css' rel='stylesheet' media='screen'>\n";
        $str .= "<script src='__SZPHP_EXTEND__/Org/bootstrap/js/bootstrap.min.js'></script>";
        $str .= '
                <!--[if lte IE 6]>
                <link rel="stylesheet" type="text/css" href="__SZPHP_EXTEND__/Org/bootstrap/ie6/css/bootstrap-ie6.css">
                <![endif]-->';
        $str .= '
                <!--[if lt IE 9]>
                <script src="__SZPHP_EXTEND__/Org/bootstrap/js/html5shiv.min.js"></script>
                <script src="__SZPHP_EXTEND__/Org/bootstrap/js/respond.min.js"></script>
                <![endif]-->';
        return $str;
    }
    ...
    {
        return "<script src='__SZPHP_EXTEND__/Org/Less/es5-shim/es5-sham.min.js'></script>\n
                <script src='__SZPHP_EXTEND__/Org/Less/es5-shim/es5-shim.min.js'></script>\n
                <script src='__SZPHP_EXTEND__/Org/Less/less-1.5.0.min.js'></script>\n";
    }

    //js轮换版
    public function _slide($attr, $content)
    {
        return "<script src='__SZPHP__/../hdjs/js/slide.js'></script>\n";
    }

    //jquery
    public function _jquery($attr, $content)
    {
        return "<script type='text/javascript' src='__SZPHP_EXTEND__/Org/Jquery/jquery-1.8.2.min.js'></script>\n";
    }

    //日历
    public function _cal($attr, $content)
    {
        return "<script src='__SZPHP__/../hdjs/org/cal/lhgcalendar.min.js'></script>\n";
    }
    ...
    {
        $php = '';
        $php .= "<script type='text/javascript' src='__SZPHP_EXTEND__/Org/Jquery/jquery-1.8.2.min.js'></script>\n";
        $php .= "<link href='__SZPHP__/../hdjs/css/hdjs.css' rel='stylesheet' media='screen'>\n";
        $php .= "<script src='__SZPHP__/../hdjs/js/hdjs.js'></script>\n";
    ...
    {
        return "<link type='text/css' rel='stylesheet' href='__SZPHP_EXTEND__/Org/jcrop/css/jquery.Jcrop.min.css'/>\n
        <script src='__SZPHP_EXTEND__/Org/jcrop/js/jquery.Jcrop.min.js'></script>\n";
    }
}
