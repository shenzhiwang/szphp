 ...
 * @return bool|string
 */
function sz_date($time, $format = 'Y-m-d')
{
 ...
 * @return string
 */
function sz_substr($string, $len = 20, $end = '...')
{
...
function file_merge($files, $space = false, $tag = false)
{
    ...
    return $tag ? '<?php if(!defined("SZPHP_PATH")){exit("No direct script access allowed");}' . $str . "\t?>" : $str;
}
 ...
 * @throws
 */
function throw_exception($msg, $type = "SzException", $code = 0)
{
...
function halt($error)
{
    ...
    require SZPHP_TPL_PATH . 'halt.html';
    exit;
}
 ...
 * @return mixed
 */
function trace($value = '[SZPHP]', $level = 'DEBUG', $record = false)
{
    static $_trace = array();
    if ('[SZPHP]' === $value) { // 获取trace信息
        return $_trace;
    } else {
