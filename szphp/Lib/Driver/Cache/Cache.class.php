...
class Cache
{
    ...
    protected function queue($name)
    {
        ...
        $_queue = $drivers[$driver][0]("SZPHP_queue");
        if (!$_queue) {
            $_queue = array();
        }
        array_push($_queue, $name);
        $SZPHP_queue = array_unique($_queue);
        //超过队列允许最大值
        if (count($SZPHP_queue) > $this->options['length']) {
            $gc = array_shift($SZPHP_queue);
            if ($gc)
                $this->del($gc);
        }
        return $drivers[$driver][0]("SZPHP_queue", $SZPHP_queue);
    }
