...
function import($class = null, $base = null, $ext = ".class.php")
{
    ...
    if (is_null($base)) {
        ...
        if ($info[0] == '@' || APP == $info[0]) {
            ...
            $base = GROUP_PATH;
            $class = substr_replace($class, '', 0, strlen($info[0]) + 1);
        } elseif (strtoupper($info[0]) == 'SZPHP') {
            $base = dirname(substr_replace($class, SZPHP_PATH, 0, 6));
...
function session($name = '', $value = '')
{
    ...
    if ($name === '') {
        ...
        if (C('SESSION_TYPE') and C('SESSION_TYPE') != 'file') { // 读取session驱动
            ...
            // 检查驱动类
            if (require_cache(SZPHP_DRIVER_PATH . '/Session/' . $class . '.class.php')) {
                $hander = new $class();
                $hander->run();
            } else {
                error("类文件不存在:$class");
...
{
    //导入类库
    require_cache(SZPHP_EXTEND_PATH . "Org/Mobile/Mobile.class.php");
