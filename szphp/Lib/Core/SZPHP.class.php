...
final class SZPHP
{
    ...
    static public function autoload($className)
    {
        ...
        if (substr($className, -5) == 'Model') {
            ...
            if (require_array(array(
                SZPHP_DRIVER_PATH . 'Db/' . $class
            ))
            ) return;
        } elseif (substr($className, 0, 5) == 'Cache') {
            if (require_array(array(
                SZPHP_DRIVER_PATH . 'Cache/' . $class,
            ))
            ) return;
        } elseif (substr($className, 0, 4) == 'View') {
            if (require_array(array(
                SZPHP_DRIVER_PATH . 'View/' . $class,
            ))
            ) return;
            ...
            SZPHP_CORE_PATH . $class,
            SZPHP_EXTEND_PATH . $class,
            SZPHP_EXTEND_PATH . '/Tool/' . $class
        ))
        ) {
            return;
        }
        ...
        switch ($errno) {
            ...
            default:
                ...
                if (DEBUG && C('SHOW_NOTICE'))
                    require SZPHP_TPL_PATH . 'notice.html';
                break;
        }
    }
