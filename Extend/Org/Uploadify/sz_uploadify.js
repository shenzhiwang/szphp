//当前文件只能加载一次
sz_uploadify_js = true;
var sz_uploadify_options = {
//    removeTimeout   : 11//提示框消失时间
    ...
    uploader: SZPHP_CONTROL + "&m=sz_uploadify"//上传处理php
    ,
    ...
    onSWFReady: function () {//swfupload准备好
        ...
        this.queueData.uploadsSuccessful = SZPHP_UPLOAD_TOTAL;
        //修改全局环境量
        this.setStats({
    ...
    onUploadSuccess: function (file, data, response) {
        ...
        //第几个上传文件，会受全局变量SZPHP_UPLOAD_TOTAL的影响
        ...
        html += "</li>";
        div.append(html);
        if (typeof(sz_upload) == 'function') {
            sz_upload(file, data, response);
        }
    }
};
...
$(".delUploadFile").live("click", function () {
    ...
    $.post(SZPHP_CONTROL + "&m=sz_uploadify_del", {
        file: delFiles
    }, function (data) {
        if (data == 1) {
            upload.setStats({
